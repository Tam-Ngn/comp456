...
key <- "&api-key=LvfvnLSlhEC4y5lgOQsmuizGy9p4MZAX"
url <- "https://api.nytimes.com/svc/search/v2/articlesearch.json?q=biden&being_date=20210101&end_date=20210101&page=1"
...
save(bidenarticlesafter, file = "bidenarticles.RData")



biden_text_after <- NULL
for (i in 1:length(bidenarticlesafter$web_url)) {
  ...
  body_text <- 
    ...
    html_text()
  body_text_coll<- tibble(url = bidenarticlesafter$webb_url[i], text = paste(body_text, collapse = " "))
  biden_text_after <- bind_rows(biden_text_after, body_text_coll)
}

save(biden_text_after, file = "biden_text_after_nyt.RData")
