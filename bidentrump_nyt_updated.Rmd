...
library(SnowballC)
data("stop_words")
mystopwords <- tibble(word = c("trump", "trumps", "trump's","trump’s", "biden", "biden's", "biden’s", "donald", "u.s", "joe", "elizabeth", "warren", "here/'s", "sanders", "joseph", "it/'s", "here's", "jr", "vice", "bernie", "obama", "hampshire", "thursday", "tuesday", "bloomberg", "ms", "gail", "bret", "dr", "buttigieg","don/'t"))
```
...
```{r}
jan_nyt_trump <- full_trump_nyt %>%
mutate(Date = as_date(pub_date)) %>%
  filter(between(Date, as.Date('2021-01-01'), as.Date('2021-01-31'))) 

jan_nyt_trump %>%
  select(Date, word) %>% 
  mutate(date = str_trunc(Date, 10, ellipsis = "")) %>% 
  mutate(date = ymd(date)) %>% 
  unnest_tokens(word, word) %>% 
  anti_join(stop_words) %>% 
  anti_join(mystopwords) %>% 
  ...
  labs(title = "New York Times January 2021",
       subtitle = "Sentiment Score for Trump by Day") +
  xlab("") +
  ylab("Score") +
  theme_classic()
...
If we Zoom in more on Trump 2021 from the NYT, first we can notice that his score is never really positive. Overall, he has a remarkably negative sentiment score overall. Secondly, we see that his score hits an all time low right around the 9th of the month, which makes sense considering that on January 6th, 2021, the insurection happened. 
...
```
During the month of January, the negative words most frequently associated with Donald Trump were "attack", "fraud", "conservative", and "lost". 
...
```{r}
full_trump_nyt %>%
  inner_join(get_sentiments("afinn")) %>% ggplot(aes(value)) + geom_histogram(color = "black", fill = "white", 
    bins = 30)
```
...
full_biden_nyt %>%
  inner_join(get_sentiments("afinn")) %>% ggplot(aes(value)) + geom_histogram(color = "black", fill = "white", 
    bins = 30) 
```
Considering scale, the count of words is much higher for Biden. Secondly, there is much more of a spread in terms of negative versus positive for Biden. There seems to be more negative words associated with Trump versus Biden over the span of two years. 
