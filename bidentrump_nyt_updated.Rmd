...
tidy_body_text_tot <- body_text_bidenbefore %>%
...
```{r}
body_text_bidenbefore %>% 
  rename(web_url = url) %>% 
  left_join(bidenarticles, by = "web_url") %>% 
  ...
  labs(title = "New York Times 2020",
...
tidy_biden <- tidy_body_text_tot_bidenafter %>% 
  unnest_tokens(word, text) %>% 
  anti_join(stop_words) %>% 
  anti_join(mystopwords) %>% 
  count(word, sort = TRUE)
...
```{r}
# Trump Before
load("C:/Users/cecei/Desktop/comp465/Articles Data/trump_nytimes_2020.RData")
load("C:/Users/cecei/Desktop/comp465/Articles Data/trump_nytimes_2020_text.RData")
```

##  Data Cleaning
```{r}
trumpbefore <- totalarticles %>% 
  unnest(headline) %>% 
  select(abstract, snippet, lead_paragraph, main, pub_date, section_name, web_url)

tidy_body_text_tot_trumpbefore <- body_text_tot %>%
  mutate(text = str_remove_all(text, "[:punct:]")) %>% 
  filter(text != "")
...
tidy_body_text_tot_trumpafter <- text_trump_after %>%
  mutate(text = str_remove_all(text, "[:punct:]")) %>% 
  filter(text != "")
