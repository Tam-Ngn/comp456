...
#finding the overall percentages
...
Overall, 73.68% of Guardian articles written about Biden is negative, compared to a 26.32% percent for postives.  

```{r stacked bar chart for Biden Guardian}
plot1 <- score_per_url %>% 
  mutate(ym = str_sub(date, 1,7 )) %>% 
  group_by(webUrl, ym) %>% 
  summarize(art_score) %>% 
  ...
  count(isPositive) %>% 
  mutate(totalArt = sum(n), percent = n/totalArt) %>% 
  filter(ym(ym) > ymd("2019-12-31"), ym(ym) < ymd("2022-02-01") ) %>% 
  ggplot(aes(y = ym, x = percent, fill = isPositive)) + 
  geom_col()+
  labs(title = "Affective Percentages of Guardian Articles on Biden by Month",
       x = "",
       y = "" )
```
...
```{r}
#getting the data ready
score_per_url_trump <- guardian_trump_full %>% 
  group_by(webPublicationDate, webUrl) %>% 
  unnest_tokens(word, text) %>% 
  mutate(date = as.Date(webPublicationDate)) %>% 
  ungroup() %>% 
...
Similarly for Trump, 74% of published articles were negative, as opposed to 26% that were positive. Compared to Biden, the statistics for Trump are only slightly worse.


```{r}
plot2 <- score_per_url_trump %>% 
  mutate(ym = str_sub(date, 1,7 )) %>% 
  group_by(webUrl, ym) %>% 
  summarize(art_score) %>% 
  ungroup() %>% 
  ...
  ggplot(aes(y = ym, x = percent, fill = isPositive)) + 
  geom_col()+
  labs(title = "Affective Percentages of Guardian Articles on Trump by Month",
       x = "",
       y = "" )
```
...
```{r put the graphs side by side fig.width= 6, fig.height= 3}
library(patchwork)
plot2 + plot1
```


A lot of blue bars fell under 0.25 for The Guardian, which further strengthens our hypothesis that the international newspaper has stronger negative feelings towards the presidential candidates. One thing they have in common though is the month of 2022-01 when no single positive articles were written about Trump. SImilarly, Biden scored the lowest positive proportion in the very same month and year . After a few google searches it was one year after the capitol insurrection, Trump was formally accused of 4 crimes and members of congress agreed he should be barred from holding office ever again  

# NYT and Guardian together

```{r}
trump_guardian_time <- full_trump_guardian %>% 
  mutate(date = str_trunc(pub_date, 10, ellipsis = "")) %>% 
  mutate(date = ymd(date)) %>%   
  inner_join(get_sentiments("afinn")) %>% 
  group_by(date, url) %>% 
...
biden_guardian_time <- full_biden_guardian %>% 
  mutate(date = str_trunc(pub_date, 10, ellipsis = "")) %>% 
  mutate(date = ymd(date)) %>%   
